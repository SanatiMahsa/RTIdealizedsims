module hydro_parameters
  ...
  ! Cosmic rays and twotemp parameters
!!$  real(dp)::mu_gas=1.0      ! For tests only
!!$  real(dp)::mu_electron=1.0 ! For tests only
!!$  real(dp)::mu_ion=1.0      ! For tests only
...
!!$  real(dp)::mu_gas=0.5      ! Fully ionised Hydrogen gas
!!$  real(dp)::mu_electron=1.0 ! Fully ionised Hydrogen gas
!!$  real(dp)::mu_ion=1.0      ! Fully ionised Hydrogen gas
!!$  real(dp)::mu_gas=1.333333 ! Fully ionised Helium gas
!!$  real(dp)::mu_electron=2.0 ! Fully ionised Helium gas
!!$  real(dp)::mu_ion=4.0      ! Fully ionised Helium gas
  ...
  real(dp)::floor_prad=1.d-10! Pressure (NENER) floor
  real(dp)::floor_var=1.d-10 ! Passive scalar floor
  ...
  real(dp)::smallb=1.d-20
  real(dp)::smallcr=1.d-30
