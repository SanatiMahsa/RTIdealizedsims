     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              if (is_active_star(idp(ipart),mp(ipart),tp(ipart))) then
                 npart2=npart2+1
              endif
  ...
  do j=1,np
     ...
     if(birth_time.le.(current_time-t0))then           
        ...
        if(write_stellar_densities) &
             st_n_SN(ind_part(j)) = unew(indp(j),1)
        ...
        if(ethermal(j) .gt. 0d0 .and. write_stellar_densities) then 
           st_e_SN(ind_part(j)) = ethermal(j) * vol_loc(j) * scale_d     &
                * scale_l**3 * scale_v**2 / 1d51
        endif
...
subroutine kinetic_feedback
  ...
  integer ,dimension(:),allocatable::ind_part_all ! Stellar density tracking
#endif
  !----------------------------------------------------------------------
  ...
  integer ,dimension(:),allocatable::ind_part,ind_part_tot ! Stellar density (SD) tracking
     ...
     do jgrid=1,numbl(icpu,levelmin)
        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              if (is_active_star(idp(ipart),mp(ipart),tp(ipart)) .and. &
  ...
  allocate(ind_part_tot(1:nSN_tot)) ; ind_part_tot(1:nSN_tot)=0
        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              if(is_active_star(idp(ipart),mp(ipart),tp(ipart)) .and. &
                 ...
                 ind_part_tot(iSN) = ipart ! For tracking SN densities
  ...
  allocate(ind_part_all(1:nSN_tot))
  ...
  call MPI_ALLREDUCE(ind_part_tot,ind_part_all,nSN_tot,MPI_INTEGER, MPI_SUM,MPI_COMM_WORLD,info)
  xSN_tot=xSN_all
  vSN_tot=vSN_all
  mSN_tot=mSN_all
  ZSN_tot=ZSN_all
  ind_part_tot=ind_part_all
  deallocate(xSN_all,vSN_all,mSN_all,ZSN_all)
  deallocate(ind_part_all)
#endif
  ...
  allocate(ind_part(1:nSN_tot)) ; ind_part(1:nSN_tot)=0
  ...
  do iSN=1,nSN
     ...
     ind_part(iSN) = ind_part_tot(isort)
  enddo
  deallocate(xSN_tot,vSN_tot,mSN_tot,ZSN_tot,itemp)
  deallocate(ind_part_tot)
  ...
  call average_SN(xSN,vSN,vol_gas,dq,ekBlast,indSN,nSN,nSN_tot,iSN_myid &
       ,mSN,mloadSN,ZSN,ZloadSN,vloadSN,ind_part)
...
!################################################################
!################################################################
subroutine average_SN(xSN,vSN,vol_gas,dq,ekBlast,ind_blast,nSN,nSN_tot &
     ,iSN_myid,mSN,mloadSN,ZSN,ZloadSN,vloadSN,ind_part)
  use pm_commons
  use amr_commons
  use hydro_commons
  implicit none
  ...
  integer ,dimension(1:nSN)::ind_part
  integer::ind_SN
  real(dp)::rcell
              ...
              if(ok(i))then
                 ...
                 do iSN=1,nSN
                    ...
                    if(dr_cell.le.dx_loc/2.0)then
                       ...
                       if(ind_part(iSN) .gt. 0 .and. write_stellar_densities) then
                          st_n_SN(ind_part(iSN)) = uold(ind_blast(iSN),1)
                          st_e_SN(ind_part(iSN)) = mSN(iSN)/(10.*2d33)   &
                                                 * scale_d * scale_l**3
                       endif
                       if(ind_part(iSN) .le. 0) &
                            print*,'PROBLEM WITH IND_PART = ',ind_part(iSN),iSN
