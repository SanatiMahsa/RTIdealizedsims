...
subroutine init_tree
  ...
  if (sink) then
     ! Reorganising the sink/cloud particles should only happen with sinks...
     ...
     do idim=1,ndim
        do j=1,np
           ...
           if(i<0.or.i>2)then
              ...
              write(*,*)"mass moved ",mp(ind_part(j)),tp(ind_part(j)),idp(ind_part(j))
           endif
        end do
     end do
     stop
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  if(star.or.sink)then
     ...
     if(write_stellar_densities) then
        do i=1,np
           st_n_tp(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property)
           st_n_SN(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property+1)
           st_e_SN(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property+2)
        end do
        current_property=current_property+3
     endif
     if(use_initial_mass)then
