...
subroutine update_time(ilevel)
  ...
  use cooling_module
#ifdef TRACKER
  use tracker_commons, only: track_quantities,track_velocity,track2logfile, &
& print_xtrack,print_vtrack,print_measuretrack
#endif
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
...
#ifdef TRACKER
           if (track2logfile) then
              call announce_read_trackers(print_xtrack)
              if (track_velocity) call announce_read_trackers(print_vtrack)
              if (track_quantities) then
                 call announce_read_trackers(print_measuretrack)
              end if
           end if
#endif
