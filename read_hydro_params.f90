...
subroutine read_hydro_params(nml_ok)
  ...
  namelist/hydro_params/gamma,courant_factor,smallr,smallc &
       & ,niter_riemann,slope_type,difmag,slope_mag_type &
#if NENER>0
       & ,gamma_rad,smallcr &
#endif
#if NCR>0
       & ,slope_limiter_aniso, CRwithStars &
       & ,debug_bicg,do_limiter_aniso,slope_limiter_aniso,alpha_limiter,epsilon_restartbicg &
       & ,explicit_diff,autoswitch_diff,sts_diff,nmax_explicit &
#endif
  ...
  namelist/refine_params/x_refine,y_refine,z_refine,r_refine &
       ...
       & ,cc_mrec,cc_pos,cc_rfac,cc_r,cc_active,cc_aini
  namelist/boundary_params/nboundary,bound_type &
       ...
       & ,J21,a_spec,z_ave,z_reion,eta_mag,ind_rsink,delayed_cooling,T2max &
       & ,self_shielding,smbh,agn,B_ave,injBfac,mhd_AGN &
       ...
       & ,star_imf, n_dc, n_gmc, nsn2mass, star_maker, t_delay, SN_dT2_min         &
#if NCR>0
       & ,CR_ave,injCRgen,injCRgenNei,injCRext,injCRextNei &
       & ,itermax_cr,conduction,conduction_ion,twotemp,isotrope_cond,slopelim_cond,k_perp,f_spitzer,cr_diffusion &
       ...
       & ,sf_virial,sf_trelax,sf_tdiss,sf_model,sf_birth_properties, sf_lam        &
       & ,local_dmax,maxlev_sf
...
105 continue
  ...
  ! Check for cc_cuts
  !--------------------------------------------------
  if ((myid.eq.1).and.cc_active) then
     write(*,'("CC region active:")')
     write(*,'("cc (m,f,r)=",3F8.5)') cc_mrec, cc_rfac, cc_r
     write(*,'("cc_pos    =",3F8.5)') cc_pos
  end if
  ...
  if(nvar<(8+nener))then
     if(myid==1)write(*,*)'Error: non-thermal energy need nvar >= ndim+2+ndim+nener'
  ...
  do i=1,nboundary
     ...
     boundary_var(i,ndim+2)=ek_bound+em_bound+P_bound(i)/(gamma-1.0d0)
  end do
  ...
  !-----------------------------------
  inener=9 ! MUST BE THIS VALUE (MHD) !!!
  imetal=nener+ndim+6
  if (ivar_refine.gt.0) then
     ! inener goes after bfields:
     ! colour irefine should go after inener, and before metals 
     ! inener=inener+1
     imetal=imetal+1
  end if
  idelay=imetal
  ...
  if(myid==1.and.hydro.and.(nvar>ndim+2)) then
     ...
     write(*,*) '   inener  = ',inener
#endif
     if(ivar_refine.gt.0)write(*,*) '   irefine = ',ivar_refine
