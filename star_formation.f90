...
subroutine star_formation(ilevel)
  ...
  real(dp) :: Bx, By, Bz
  real(dp) :: invbeta, betafunc, p_mag
#endif
  ...
  if ((maxlev_sf).and.(ilevel.lt.nlevelmax_current)) return
  ...
  if(log_mstarmin)then
     if(myid==1) write(*,*) ">>>mstar: Mstar,min=",mstar_nsn*scale_msun,fstar_min,mstar,mass_sph,n_star,vol_min
  end if
...
! get values of uold for density and velocities in virtual boundaries
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
           ...
           do i=1,ngrid
              ...
              if (ok(i)) then 
                 ...
                 if (d <= d_gmc) then
                    ...
                    do irad = 0,nener-1
                          temp = temp + uold(ind_cell(i),inener+irad) / d * (gamma_rad(irad)-1.0) * gamma_rad(irad)
                    end do
#endif
           ...
           do i=1,ngrid
              ...
              if (ok(i)) then 
                 ...
                 if (d <= d_gmc) then
                    ...
                    sigs  = log(1.0+0.16*trgv/c_s2)
                    scrit = log(0.067/theta**2*alpha0*trgv/c_s2)
                    sfr_ff(i) = e_cts/2.0*phi_t*exp(3.0/8.0*sigs)*(2.0-erfc((sigs-scrit)/sqrt(2.0*sigs)))
                    alpha_fk(i)=alpha0
                 endif ! if d>d_gmc
              endif ! if leaf cell
           enddo
        else if(TRIM(star_maker)=='federrath3mag')then
           ! trgv for turbulence, no Jeans length criterion to form stars
           do i=1,ngrid
              ! if cell is a leaf cell
              if (ok(i)) then 
                 ...
                 if (d <= d_gmc) then
                    ...
                    c_s2  = temp    
#ifdef SOLVERmhd
                    Bx = (uold(ind_cell(i),6)+uold(ind_cell(i),nvar+1))
                    By = (uold(ind_cell(i),7)+uold(ind_cell(i),nvar+2))
                    Bz = (uold(ind_cell(i),8)+uold(ind_cell(i),nvar+3))
                    p_mag = 0.125d0*(Bx**2 + By**2 + Bz**2) 
                    invbeta = p_mag/(c_s2*d) 
                    c_s2 = c_s2*(1.+invbeta) 
#endif
                    alpha0 = 5d0/(onepi*factG*d)*(trgv + c_s2)/dx_loc**2
#ifdef SOLVERmhd
                    e_cts = 0.5
                    phi_t = 0.57 ; theta = 0.33
                    betafunc = ((1.+0.925*invbeta**1.5)**(2./3.)) / ((1.+invbeta)**2)
                    scrit = log(0.067*betafunc/(theta**2)*alpha0*trgv/c_s2)
#else
                    e_cts = 0.5
                    phi_t = 0.57 ; theta = 0.33
                    scrit = log(0.067/theta**2*alpha0*trgv/c_s2)
        ...
        else if(TRIM(star_maker)=='FKmag')then
           ! Enforce turbulence criterion + efficiency following Federrath & Klessen 2012
           do i=1,ngrid
              ! if cell is a leaf cell and high density
              d = uold(ind_cell(i),1)
              ...
              if (ok(i)) then 
                 ...
                 if(ok(i)) then
                    ...
                    By = (uold(ind_cell(i),7)+uold(ind_cell(i),nvar+2))
                    Bz = (uold(ind_cell(i),8)+uold(ind_cell(i),nvar+3))
                    ! 1/2 factor in p_mag comes from (Br+Bl)/2
                    p_mag = 0.125d0*(Bx**2 + By**2 + Bz**2) ! p_mag = B^2/2
                    invbeta = p_mag/(c_s2*d) ! beta = c_s^2 * rho / p_mag
        ...
        do i=1,ngrid
           ...
           if (ok(i)) then
              ...
              if (mgas > 0.9*mcell) then
                 ...
                 write(*,*)"gas depletion",mstar,nstar_corrected,nstar(i),mstar_lost
              ...
              if (nstar(i)>0) then
                 ntot = ntot+1
                 if(log_sf)then
399                  format('SF z= ',f9.5,1x,' nH= ',f7.3,' N= ',I5,' eps_ff= ',f9.5,' alpha= ',f9.5,' mstar= ',E12.5,' mgas= ',E12.5)
                     write(*,399) 1./aexp-1,log10(d*scale_nH),nstar(i),sfr_ff(i),alpha_fk(i), mstar, mgas
                 endif
              else
                 nstar(i)=0
              endif
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
           ...
           idp(ind_part(i))    = -index_star-nsinkmax_safe-1  ! Star identity
