...
subroutine create_sink
  ...
  if (nsinkmax_safe.lt.nsinkmax) then
   write(*, '(": Error! nsinkmax_safe should be larger than nsinkmax from the beginning of the simulation")')
   stop 99
end if

  ! Conversion factor from user units to cgs units
  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  rho_min_acc_code=rho_min_acc/scale_d
...
#ifdef TRACKER
  ! Seed SMBH using center of mass tracking
  if ( &
       & (seed_smbh_com.or.fix_sink2com) &
       & .and.track_center_mass &
       & ) then
     call compute_track_center_mass_position(.true.)
  end if
  
  ! Relocates the sink to the center of mass
  PerSink: do isink=1,nsink     
     if (fix_sink2com) then
        xsink(isink,1:ndim)=tracker(1)%xpos
        ! vsink(isink,1:ndim)=vtrack_center_mass(1:ndim)
     end if
  end do PerSink
#endif
     ...
     do jgrid=1,ncache
        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              if((idp(ipart).lt.0).and.(tp(ipart).eq.0.0d0))then
                 npart2=npart2+1
              endif
...
subroutine collect_acczone_avg(ilevel)
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart2>0)then
           ...
           do jpart=1,npart1
              ...
              if(ip==nvector)then              
                 ! if(ip==nvector)then
                 if (ip.gt.0) call collect_acczone_avg_np(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel)
                 ip=0
                 ig=0
              end if
              ipart=next_part  ! Go to next particle
...
#endif
        call MPI_ALLREDUCE(wmetal,wmetal_new,nsinkmax,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
     end if
#else
     wden_new=wden
     wvol_new=wvol
  ...
  do isink=1,nsink
     ...
     if (detailedSMBH) then
        weighted_emag(isink,ilevel)=wemag_new(isink)        
        weighted_metal(isink,ilevel)=wmetal_new(isink)
     end if
  end do
...
subroutine collect_acczone_avg_np(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
...
#endif
  real(dp),dimension(1:nvector),save::metalc
  ...
  do j=1,np
     ...
     if(d>0.)then
        ...
        emag(j)=0.125d0*((bx1+bx2)**2+(by1+by2)**2+(bz1+bz2)**2)
        e=e-emag(j)
#endif       
        ...
        emag(j)=0.
#endif
        if (floor_acc) d=max(d-rho_min_acc_code,rho_min_acc_code)
     end if
  end do
...
#ifdef SOLVERmhd
     wemag(isink)=wemag(isink)+weight*emag(j)
#endif
     wmetal(isink)=wmetal(isink)+weight*metalc(j)
  end do
...
subroutine grow_sink(ilevel,on_creation)
  ...
  real(dp),allocatable,dimension(:)::agn_inj_mag_out,agn_inj_tot_out
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart2>0)then        
           ...
           do jpart=1,npart1
              ...
              if(ip==nvector)then              
                 !if(ip==nvector)then
                 if (ip.gt.0) call accrete_sink(ind_grid,ind_part,ind_grid_part,ig,ip,ilevel,on_creation)
                 ip=0
                 ig=0
              end if
              ipart=next_part  ! Go to next particle
  ...
  if(nsink>0)then
#ifndef WITHOUTMPI
     if(.not.allocated(agn_inj_mag_out))then
      allocate(agn_inj_mag_out(1:nsinkmax))
     ...
     if (detailedSMBH) then
        call MPI_ALLREDUCE(lmom_gas_new,lmom_gas_all,nsinkmax*3,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
     end if  
#else
     msink_all=msink_new
...
subroutine accrete_sink(ind_grid,ind_part,ind_grid_part,ng,np,ilevel,on_creation)
  ...
  real(dp)::bx1,bx2,by1,by2,bz1,bz2
  real(dp)::emag_AGN
  real(dp)::emaginject_AGN
  logical::log_inject_AGN=.false.
  integer::igrid
#endif
#ifdef RT
  integer::igroup
  ...
  real(dp)::fbk_ener_AGN_store,fbk_mom_AGN_store,fbk_tot_AGN_store
  logical,dimension(1:ndim)::period
  ...
  scale=boxlen/dble(nx_loc)
  dx_loc=dx*scale
  vol_loc=max(dx_loc**ndim,tiny(0.0d0))
  dx_min=scale*0.5D0**nlevelmax/aexp
  vol_min=dx_min**ndim
  ...
  scale_evtocode=1.6021766d-12/(scale_d*scale_l**5/scale_t**2)
#endif
  ...
  do ind=1,twotondim
     do j=1,np
        if(ok(j,ind))then
           ...
           density=max(rho_gas(isink),tiny(0.0d0))
           volume=max(volume_gas(isink),tiny(0.0d0))
...
!#endif
              ...
              !Switch between quasar and radio mode
              if(agn.and.msink(isink).gt.0)then
                 acc_ratio=dMsink_overdt(isink)/(4.*3.1415926*6.67d-8*msink(isink)*1.66d-24/(0.1d0*6.652d-25*3d10)*scale_t)
                 if (AGN_fbk_mode_switch_threshold > 0.0) then
                    !Merloni & Heinz 2008 feedback modes
                    if (acc_ratio < AGN_fbk_mode_switch_threshold) then
                       ! Eddington ratio lower than AGN_fbk_mode_switch_threshold -> kinetic
                       ! Eddington ratio higher than AGN_fbk_mode_switch_threshold -> thermal+kinetic
                       AGN_fbk_frac_ener = 1.0d-10
                       AGN_fbk_frac_RT = 1.0d-10
                       !AGN_fbk_frac_mom = 1
                    end if
                 end if
...
!#endif
           ...
           if(nol_accretion)then
              p_rel_acc=p_rel_rad*acc_mass/(d*vol_loc+tiny(0.0d0))
           else
              p_rel_acc=p_rel*acc_mass/(d*vol_loc+tiny(0.0d0))
           end if
           ...
           if (8*acc_mass/(vol_loc+tiny(0.0d0))>d .and. (.not. on_creation))then 
              write(*,*)'====================================================='
           ...
           if (on_creation)then
              ...
              d=d-acc_mass/(vol_loc+tiny(0.0d0))
              !new gas velocity
              vv(1:3)=(d*vol_loc*vv(1:3)-p_acc(1:3))/max(d*vol_loc-acc_mass,tiny(0.0d0))
              ...
              if (floor_acc) unew(indp(j,ind),1)=max(unew(indp(j,ind),1),rho_min_acc_code)
                ...
                if(ok_blast_agn(isink).and.delta_mass(isink)>0.0)then
                  ...
                  if(AGN_fbk_frac_mom.gt.0.0)then ! momentum AGN feedback
                     ...
                     if (orth_dist.le.abs(cone_dist)*tan_theta)then
                        fbk_mom_AGN_store = sum(fbk_mom_AGN*r_rel(1:3)/(ir_cloud*dx_min)*vv(1:3))/vol_loc 
                  ...
                  if(rt_AGN)then
                     if(AGN_fbk_frac_RT.gt.0.0)then 
                        dert=AGN_fbk_frac_RT*0.1d0*delta_mass(isink)*(3.d10/scale_v)**2
                        !write(*,*)'rt_AGN',AGN_fbk_frac_RT
                        do igroup=1,nGroups
...
subroutine compute_accretion_rate(write_sinks)
  ...
  do isink=1,nsink
     ...
     do i=levelmin,nlevelmax
        ...
        if (detailedSMBH) then
           emag=emag+weighted_emag(isink,i)
           metalc=metalc+weighted_metal(isink,i)
           ethermal0=ethermal
        end if
     end do
     mgas=density
...
!        write(*,'("compute_accretion:",4(1X,1PE14.7))'),vsink(isink,1:3),v2
        ...
        if (detailedSMBH) then
           emag_sink(isink)=emag
           eth_sink(isink)=ethermal0
           metal_sink(isink)=metalc
        end if
...
subroutine print_sink_properties(dMEDoverdt,rho_inf,r2,v_bondi)
  ...
  integer::i,isink
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,scale_m
  real(dp)::scale_B,scale_e
  real(dp)::l_abs
  real(dp)::mDFsum1,mDFsum2
  ...
  SMBHPrint: if (smbh) then
     OnePrint: if(myid==1.and.nsink>0)then
        xmsink(1:nsink)=msink(1:nsink)
        call quick_sort_dp(xmsink(1),idsink_sort(1),nsink)
        write(*,'(" Number of sink = ",I7,". aexp=",F10.7)') nsink, aexp
        ...
        do i=nsink,max(nsink-10,1),-1          
           isink=idsink_sort(i)
           ! This needs reorganising into a more sensible format where interesting
           ! information can be extracted from each line (also should have sinkID and redshift in all lines)
           
           write(*,'("Sink:",I7,", ",I7)') i, isink
           ! Writes: MBH (Msun), M_Bondi(Msun/yr), M_Edd (Msun/yr), pos [x,y,z] (code units), M_acc_last (Msun)
           write(*,'("OS1:",10(1X,1PE14.7))') msink(isink)*scale_m/2d33 &
                & ,dMBHoverdt(isink)*scale_m/scale_t/(2d33/(365.*24.*3600.)) &
                & ,dMEDoverdt(isink)*scale_m/scale_t/(2d33/(365.*24.*3600.)) &
                & ,xsink(1,1:ndim),delta_mass(isink)*scale_m/2d33
           ! Writes: rho_gas (g/cc), rho_gas at infinity (g/cc), m_gas(Msun), sound speed (km/s), rBondi (pc)
           write(*,'("OS2:",12(1X,1PE14.7))') rho_gas(isink)*scale_d,rinfsink(isink)*scale_d &
                & ,rho_gas(isink)*volume_gas(isink)*scale_m/2d33,sqrt(c2sink(isink))*scale_v/1e5 &
                & ,sqrt(r2sink(isink))*scale_l/3.086e18
           ...
           ! Writes:
           ! <e_thermal>  (erg/cc), <e_mag>  (erg/cc), metallicity (fraction), <(rho*v - rho*vsink)> (g*cm/s)
           if (drag_part) then
              mDFsum1=sum(mass_DF(isink,:,1))
              mDFsum2=sum(mass_DF(isink,:,2))
              write(*,'("OS4:",6(1X,1PE14.7))') eth_sink(isink)*scale_e, emag_sink(isink)*scale_e, metal_sink(isink), wdiv(isink) &
                   & ,mDFsum1*scale_m/2d33,mDFsum2*scale_m/2d33
           else
              write(*,'("OS4:",4(1X,1PE14.7))') eth_sink(isink)*scale_e, emag_sink(isink)*scale_e, metal_sink(isink), wdiv(isink)
           end if
        end do
...
subroutine make_sink_from_clump(ilevel)
  ...
  ! Hugo's particles dynamical friction
  if(drag_part) then
...
! updated by summing the contributions from all levels.                      
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  real(dp)::dteff
  integer::lev,isink,idim
   ...
   fsink=0.
     ...
     PerSink: do isink=1,nsink
        ...
        do lev=levelmin,nlevelmax
           ...
           !write(*,*)"fsink",fsink(isink,1:ndim),fsink_partial(isink,1:ndim,lev),vsink(1:nsink,1:ndim)
        end do
        if (.not. direct_force_sink(isink))then
           fsink(isink,1:ndim)=fsink(isink,1:ndim)/(dble(ncloud_sink)+tiny(0.0d0))
        end if
...
!#endif
        ...
        !write(*,'("BSUP:",3(1X,1PE14.7))'), xsink(isink,1:ndim)
  ...
  do jgrid=1,numbl(myid,ilevel)
     ...
     if(npart1>0)then
        ...
        do jpart=1,npart1
           ...
           ! ONLY_CLOUD: if ((idp(ipart).lt.0).and.(tp(ipart).eq.0.0d0)) then
              if(ig==0)then
                 ig=1
                 ind_grid(ig)=igrid
              end if              
           ...
           ! end if ONLY_CLOUD
           if(ip==nvector)then              
              if (ip.gt.0) call upd_cloud(ind_part,ip)
              ip=0
              ig=0
           end if
           ipart=next_part  ! Go to next particle
...
subroutine upd_cloud(ind_part,np)
  ...
  do j=1,np
     isink=-idp(ind_part(j))
     ! if((isink.gt.0).and.(mp(ind_part(j))>0.) then
     if((idp(ind_part(j)).lt.0).and.(mp(ind_part(j))>0.).and.&
          & (tp(ind_part(j)).eq.0.0d0)) then
     ...
     if (msink(isink)>-1.)then
        do jsink=isink+1,nsink
           ...
           if (merge)then
              ...
              mergers=mergers+1
              mnew=msink(isink)+msink(jsink)+tiny(0.0d0)
              ! COM
        ...
        do jsink=isink+1,nsink
           ...
           if (mass_merger_vel_check_AGN>0)then
              if((msink(isink)+msink(jsink)).ge.mass_merger_vel_check_AGN*2d33/(scale_d*scale_l**3)) then
                 ...
                 merge=merge .and. 2*factG*(msink(isink)+msink(jsink))/sqrt(rr + tiny(0.0d0))>v1_v2
              end if
           end if
           ...
           if (merge)then
              ...
              if (drag_part) then
                 do ii = levelmin, nlevelmax
                    do itype = 1, 2
                       do idim = 1, ndim
                          v_DF(isink, ii, idim, itype) = &
                               & (v_DF(isink, ii, idim, itype)*msink(isink) + &
                               & v_DF(jsink, ii, idim, itype)*msink(jsink))/mnew
                       end do
                       mass_DF(isink, ii, itype) = mass_DF(isink, ii, itype) + &
                            & mass_DF(jsink, ii, itype)
                       n_part(isink, ii, itype) = n_part(isink, ii, itype) + &
                            & n_part(jsink, ii, itype)
                       mass_lowspeed(isink, ii, itype) = mass_lowspeed(isink, ii, itype) + &
                            & mass_lowspeed(jsink, ii, itype)
                       fact_fast(isink, ii, itype) = fact_fast(isink, ii, itype) + &
                            & fact_fast(jsink, ii, itype)
                    end do
                 end do
              end if
  ...
  do while (mergers>0)
     ...
     if (msink(i)<-1.)then !if sink has been merged to another one        
        ...
        do j=i,nsink
           ...
           ! Particles dynamical friction
           if (drag_part) then
              v_DF(j, levelmin:nlevelmax, 1:ndim, 1:2) = v_DFnew_all(j+1, levelmin:nlevelmax, 1:ndim, 1:2)
              mass_DF(j, levelmin:nlevelmax, 1:2) = mass_DFnew_all(j+1, levelmin:nlevelmax, 1:2)
              n_part(j, levelmin:nlevelmax, 1:2) = n_partnew_all(j+1, levelmin:nlevelmax, 1:2)
              mass_lowspeed(j, levelmin:nlevelmax, 1:2) = mass_lowspeednew_all(j+1, levelmin:nlevelmax, 1:2)
              fact_fast(j, levelmin:nlevelmax, 1:2) = fact_fastnew_all(j+1, levelmin:nlevelmax, 1:2)
           end if           
        end do
        ...
        ! Particles dynamical friction
        if (drag_part) then
           v_DF(nsink+1, levelmin:nlevelmax, 1:ndim, 1:2) = 0.0d0
           mass_DF(nsink+1, levelmin:nlevelmax, 1:2) = tiny(0.0d0)
           n_part(nsink+1, levelmin:nlevelmax, 1:2) = 0.0d0
           mass_lowspeed(nsink+1, levelmin:nlevelmax, 1:2) = 0.0d0
           fact_fast(nsink+1, levelmin:nlevelmax, 1:2) = 0.0d0
        end if
     else
        i=i+1
     end if
...
!  fsink=0.
  ...
  do isink=1,nsink
     if (direct_force_sink(isink))then
        ...
        do jsink=1,nsink
           if (direct_force_sink(jsink))then
              ff(jsink,1:ndim)=factG*msink(jsink)/(ssoft**2+d2(jsink)+tiny(0.0d0))**1.5*ff(jsink,1:ndim)
           end if
        end do
        do jsink=1,nsink           
...
subroutine read_sink_params()
  ...
  namelist/sink_params/n_sink,rho_sink,d_sink,accretion_scheme,nol_accretion,merging_timescale &
       & ,ir_cloud_massive,sink_soft,mass_sink_direct_force,ir_cloud,nsinkmax,c_acc &
       & ,create_sinks,mass_sink_seed &
       & ,eddington_limit,sink_drag,acc_sink_boost,mass_merger_vel_check_AGN &
       & ,clump_core,verbose_AGN,T2_AGN,v_AGN,cone_opening,mass_halo_AGN,mass_clump_AGN &
       & ,AGN_fbk_frac_ener,AGN_fbk_frac_mom,AGN_fbk_frac_RT,AGN_fbk_frac_mhd,AGN_fbk_mode_switch_threshold &
#ifdef TRACKER
       & ,seed_smbh_com,nsink_com,smbh_mass_track_mass,mass_sink_seed,fix_sink2com &
#endif
       & ,drag_part,DF_ncells &
       &, detailedSMBH 
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
...
!                 call count_clouds_np(ind_part,ip,action,ilevel)
                 if (ip.gt.0) call count_clouds_np(ind_grid,ind_part,ind_grid_part,ig,ip,action,ilevel)
                 ip=0
                 ig=0
              end if
              ipart=next_part  ! Go to next particle
...
subroutine cic_get_cells(indp,xx,vol,ok,ind_grid,xpart,ind_grid_part,ng,np,ilevel)
  ...
  do idim=1,ndim
     do j=1,np
        ...
        ! if (x(j,idim)<0.0 .or. x(j,idim)>6.)then
           print*,'particle outside allowed boundary for cic_get_cell'
           write(*,*) x(j,1:ndim), xpart(j,1:ndim)
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart1>0)then
           ...
           do jpart=1, npart1
              ...
              if ((tp(ipart).ne.0.d0).and.(mp(ipart).ne.0.0d0)) then
                 do ii = 1, nsink_cell
                    isink = sink_cell(ii)
                 ...
                 !DM case (2)
              else if ((idp(ipart).gt.0).and.(tp(ipart).eq.0.d0).and.(mp(ipart).ne.0.0d0)) then
  ...
  do isink = 1 ,nsink
     do itype = 1,2
        ...
        if (CoulombLog .gt. 1) then
           factor = 4*pi*factG**2*msink(isink)/(vrel_sink_norm(isink, itype)+tiny(0.0d0))**3 * &
                & (mass_lowspeedall(isink, itype)*log(CoulombLog)+&
                &  fact_fastall(isink, itype)) / max(volume,tiny(0.0d0))
        else
           factor = 0
        end if
        ...
        if(npart1>0)then
           ...
           do jpart=1, npart1
              ...
              ! if (is_cloud(typep(ipart))) then
                 isink = -idp(ipart)
                 do itype = 1, 2
                    b90 = factG * msink(isink) / (vrel_sink_norm(isink, itype)+tiny(0.0d0))**2
                    Rsh = factG * msink(isink) / c**2
                    bmin = max(b90, Rsh) + tiny(0.0d0)
                    if (itype.eq.1) then
                       CoulombLog = DF_ncells*dx_min / bmin
                    ...
                    if (CoulombLog .gt. 1) then
                       ...
                       do ii = levelmin, nlevelmax
                          factor = factor + (&
                               ...
                               &fact_fast(isink, ii, itype)) / max(volume,tiny(0.0d0))
                       end do
                       factor = factor * 4*pi*factG**2*msink(isink)/(vrel_sink_norm(isink, itype)+tiny(0.0d0))**3
                    else
                       factor = 0
                    end if
                    if (dtnew(ilevel) > 0) then
