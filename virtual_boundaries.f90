...
subroutine make_virtual_fine_rtdp(xx,ilevel)
  ...
  if(rtdp .eq. 8) then
     do icpu=1,ncpu
        ...
        if(ncache>0) then
           countrecv=countrecv+1
           call MPI_IRECV(reception(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_DOUBLE_PRECISION,icpu-1,tag,MPI_COMM_WORLD,reqrecv(countrecv),info)
        end if
     end do
  else
     do icpu=1,ncpu
        ...
        if(ncache>0) then
           countrecv=countrecv+1
           call MPI_IRECV(reception(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_REAL,icpu-1,tag,MPI_COMM_WORLD,reqrecv(countrecv),info)
        end if
     end do
  end if
  ...
  do icpu=1,ncpu
    if (emission(icpu,ilevel)%ngrid>0) then
      do j=1,twotondim
        ...
        iskip=ncoarse+(j-1)*ngridmax
        do i=1,emission(icpu,ilevel)%ngrid
          emission(icpu,ilevel)%u_RT(i+step,1)=xx(emission(icpu,ilevel)%igrid(i)+iskip)
        end do
      end do
    end if
  end do
  ...
  if(rtdp.eq.8) then
     do icpu=1,ncpu
        ...
        if(ncache>0) then
           countsend=countsend+1
           call MPI_ISEND(emission(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_DOUBLE_PRECISION,icpu-1,tag,MPI_COMM_WORLD,reqsend(countsend),info)
        end if
     end do
  else
     do icpu=1,ncpu
        ...
        if(ncache>0) then
           countsend=countsend+1
           call MPI_ISEND(emission(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_REAL,icpu-1,tag,MPI_COMM_WORLD,reqsend(countsend),info)
        end if
     end do
  end if
  ...
  do icpu=1,ncpu
    if (reception(icpu,ilevel)%ngrid>0) then
      do j=1,twotondim
        ...
        iskip=ncoarse+(j-1)*ngridmax
        do i=1,reception(icpu,ilevel)%ngrid
          xx(reception(icpu,ilevel)%igrid(i)+iskip)=reception(icpu,ilevel)%u_RT(i+step,1)
        end do
      end do 
    end if
  end do
 ...
 ! Gather emission array
     ...
     if (reception(icpu,ilevel)%ngrid>0) then
        do j=1,twotondim
           ...
           do i=1,reception(icpu,ilevel)%ngrid
              icell=reception(icpu,ilevel)%igrid(i)+iskip
              ibuf=i+step
              reception(icpu,ilevel)%u_RT(ibuf,1)=xx(icell)
           end do
        end do
     end if
  end do
  ...
  if(rtdp.eq.8) then
     do icpu=1,myid-1
        ...
        if(ncache>0) then
           ...
           ! request to send
           call MPI_SEND(countrecv,0, MPI_INTEGER, icpu-1,101,MPI_COMM_WORLD,info)
           call MPI_RECV(emission(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_DOUBLE_PRECISION,icpu-1,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE,info)
        end if
     end do
  else
     do icpu=1,myid-1
        ...
        if(ncache>0) then
           ...
           call MPI_SEND(countrecv,0, MPI_INTEGER, icpu-1,101,MPI_COMM_WORLD,info)
           call MPI_RECV(emission(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_REAL,icpu-1,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE,info)
        end if
     end do
  end if
  ...
  if(rtdp.eq.8) then
     do icpu=1,ncpu
        ...
        if(ncache>0) then
           ...
           call MPI_RECV(countrecv,0, MPI_INTEGER, icpu-1,101,MPI_COMM_WORLD, &
                & MPI_STATUS_IGNORE, info)
           call MPI_SEND(reception(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_DOUBLE_PRECISION,icpu-1,tag,MPI_COMM_WORLD,info)
        end if
     end do
  else
     do icpu=1,ncpu
        ...
        if(ncache>0) then
           ...
           call MPI_RECV(countrecv,0, MPI_INTEGER, icpu-1,101,MPI_COMM_WORLD, &
                & MPI_STATUS_IGNORE, info)
           call MPI_SEND(reception(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_REAL,icpu-1,tag,MPI_COMM_WORLD,info)
        end if
     end do
  end if
  ...
  do icpu=1,ncpu
     if (emission(icpu,ilevel)%ngrid>0) then
        do j=1,twotondim
           ...
           do i=1,emission(icpu,ilevel)%ngrid
              xx(emission(icpu,ilevel)%igrid(i)+iskip)= &
                   & xx(emission(icpu,ilevel)%igrid(i)+iskip) + emission(icpu,ilevel)%u_RT(i+step,1)
           end do
        end do
     end if
  end do
  ...
  if(rtdp.eq.8) then   
     do icpu=1,ncpu
        ...
        if(ncache>0) then
           countrecv=countrecv+1
           call MPI_IRECV(emission(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_DOUBLE_PRECISION,icpu-1,tag,MPI_COMM_WORLD,reqrecv(countrecv),info)
        end if
     end do
  else
  ...
  do icpu=1,ncpu
     if (reception(icpu,ilevel)%ngrid>0) then
        do j=1,twotondim
           ...
           iskip=ncoarse+(j-1)*ngridmax
           do i=1,reception(icpu,ilevel)%ngrid
              reception(icpu,ilevel)%u_RT(i+step,1)=xx(reception(icpu,ilevel)%igrid(i)+iskip)
           end do
        end do
     end if
  end do
  ...
  if(rtdp.eq.8) then
     do icpu=1,ncpu
        ...
        if(ncache>0) then
           countsend=countsend+1
           call MPI_ISEND(reception(icpu,ilevel)%u_RT,ncache*twotondim, &
                & MPI_DOUBLE_PRECISION,icpu-1,tag,MPI_COMM_WORLD,reqsend(countsend),info)
        end if
     end do
  else
...
#ifdef RT
         deallocate(emission(icpu,ilevel)%u_RT)
#endif
      end if
      if(reception(icpu,ilevel)%ngrid>0)then
...
#ifdef RT
         deallocate(reception(icpu,ilevel)%u_RT)
#endif
...
#ifdef RT
         allocate(emission(icpu,ilevel)%u_RT(1:ncache*twotondim,1:1))
#endif
     endif
     ncache=reception(icpu,ilevel)%ngrid
     if(ncache>0)then
...
#ifdef RT
         allocate(reception(icpu,ilevel)%u_RT(1:ncache*twotondim,1:1))
#endif
     endif
  end do
...
!################################################################
!################################################################
subroutine make_virtual_reverse_int_dmf(xx,ilevel)
  use amr_commons
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
  ...
  do icpu=1,ncpu
     if (emission(icpu,ilevel)%ngrid>0) then
        do j=1,twotondim
           ...
           do i=1,emission(icpu,ilevel)%ngrid
              xx(emission(icpu,ilevel)%igrid(i)+iskip)= &
                   & max(xx(emission(icpu,ilevel)%igrid(i)+iskip),emission(icpu,ilevel)%f(i+step,1))
           end do
        end do
     end if
  end do
...
end subroutine make_virtual_reverse_int_dmf
!################################################################
!################################################################
!################################################################
!################################################################
...
!################################################################
!################################################################
subroutine make_virtual_reverse_dp_dmf(xx,ilevel)
  use amr_commons
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
...
end subroutine make_virtual_reverse_dp_dmf
!################################################################
!################################################################
!################################################################
!################################################################
