...
#ifdef NMAGTRACERS
  write(*,*) 'This routine is incompatible with MAGTRACERS'
  call clean_stop
#endif
...
!###########################################################
!###########################################################
! This routine accounts for cosmological expansion of the
! magnetic field and cosmic rays 
subroutine maghubble
  use amr_commons
  use hydro_commons
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
#endif
  integer::ilevel,icycle,nsubdiff,ivar,irad,i,iskip,ind,info,neul=5
  real(dp)::d,u,v,w,e
  real(dp)::A,B,C,e_mag,Hfactor,dtmag
#ifdef NMAGTRACERS
  integer::kk
#endif

  ! real(dp),parameter::smallrPhys=5.0d-31
  ! real(dp),parameter::smallrPhys2=0.0d0
  ! For Charlie
  real(dp),parameter::smallrPhys=1.0d-30
  real(dp),parameter::smallrPhys2=5.0d-30
  ! real(dp),parameter::smallrPhys=5.0d-31
  ! real(dp),parameter::smallrPhys2=1.0d-30
  real(dp),parameter::largeB=1.0d-7 
  logical,parameter ::keepTemp=.false.
  real(dp)::smallp,smallr_code,smallr_code2,large_emag
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,scale_B
  real(dp)::fexpand_cr,fexpand_mag
  ! Compute code units for density floor
  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2) 
  scale_B = (sqrt(scale_d) * scale_l / scale_t)
  large_emag = 0.5*(largeB / scale_B)**2.0
  smallr_code = smallrPhys / scale_d
  smallr_code2 = smallrPhys2 / scale_d
  smallp = smallr_code*smallc**2
  Hfactor=hexp
  dtmag  =dtnew(levelmin)
  fexpand_mag=1.0d0 + 0.5d0*Hfactor*dtmag
  ...
  do ilevel=nlevelmax,levelmin,-1
     ...
     call make_virtual_reverse_dp(unew(1,nvar+3),ilevel)
#ifdef NMAGTRACERS
     do kk=nvar+3+1,nvar+3+(6*NMAGTRACERS)
        call make_virtual_reverse_dp(unew(1,kk),ilevel)
     enddo
#endif
#if NCR>0
     call make_virtual_reverse_dp(unew(1,inener),ilevel)
     fexpand_cr=1.0d0 + (5.0d0-3.0d0*gamma_rad(1))*Hfactor*dtmag
#endif
     ...
     do ind=1,twotondim
        ...
        ! H.B/2 source term in the induction equation (supercomoving coordinates)                                 
        do ivar=1,3
           do i=1,active(ilevel)%ngrid
              uold(active(ilevel)%igrid(i)+iskip,ivar+neul) = unew(active(ilevel)%igrid(i)+iskip,ivar+neul) * fexpand_mag
              uold(active(ilevel)%igrid(i)+iskip,ivar+nvar) = unew(active(ilevel)%igrid(i)+iskip,ivar+nvar) * fexpand_mag
#ifdef NMAGTRACERS
              do kk=1,NMAGTRACERS
                 uold(active(ilevel)%igrid(i)+iskip,nvar+3+ivar+(6*(kk-1))) = unew(active(ilevel)%igrid(i)+iskip,nvar+3+ivar+(6*(kk-1))) * fexpand_mag
                 uold(active(ilevel)%igrid(i)+iskip,nvar+3+3+ivar+(6*(kk-1))) = unew(active(ilevel)%igrid(i)+iskip,nvar+3+3+ivar+(6*(kk-1))) * fexpand_mag
              enddo
#endif
           end do
        end do

        ! (1 + H.B**2/2)**2 source term in the energy equation (supercomoving coordinates)
        do i=1,active(ilevel)%ngrid

           ! Extract current gas density and thermal energy
           ...
           e=unew(active(ilevel)%igrid(i)+iskip,5)
           ! Remove previous kinetic energy
           e=e-0.5d0*d*(u**2+v**2+w**2)
           ! Remove previous magnetic energy
           A=unew(active(ilevel)%igrid(i)+iskip,6)+unew(active(ilevel)%igrid(i)+iskip,nvar+1)
           B=unew(active(ilevel)%igrid(i)+iskip,7)+unew(active(ilevel)%igrid(i)+iskip,nvar+2)
           C=unew(active(ilevel)%igrid(i)+iskip,8)+unew(active(ilevel)%igrid(i)+iskip,nvar+3)
           e_mag=0.125d0*(A**2+B**2+C**2)
           e = e - e_mag
#if NENER>0
           ! Remove relativistic energies
           do irad=0,nener-1
              e = e - unew(active(ilevel)%igrid(i)+iskip,inener+irad)
           end do
#endif

#if NCR>0
           ! Expand cosmic rays energy density with anti-drag Hubble term and update their contribution to cell energy
           ! NOTE: Assuming only one cosmic rays species, stored in gamma_rad(1)!!
           uold(active(ilevel)%igrid(i)+iskip,inener)=unew(active(ilevel)%igrid(i)+iskip,inener)*fexpand_cr           
#endif
           ! Modify gas density conserving temperature
111        format("ERROR: NaN found for ",A," in maghubble")
           if (isnan(d)) then
              d = smallr_code
              write(*,111) "density"
           end if
           ...
           if (isnan(u).or.isnan(v).or.isnan(w)) then
              write(*,111) "velocity"
              write(*,*) "Stopping",u,v,w
              stop
           end if
           if (d .lt. smallr_code) then
              if (keepTemp) e=e/d
              d = smallr_code
              if (keepTemp) e=e*d
           else if ((d .lt. smallr_code2).and.(e_mag.gt.large_emag)) then
              if (keepTemp) e=e/d
              d = smallr_code2
              if (keepTemp) e=e*d
           end if
           if (e.lt.smallp) e=smallp

           ! Rebuild total energy
#if NENER>0
           ! Contribution from relativistic energies
           do irad=0,nener-1              
           ...
           ! Include new and exact contribution from magnetic energy
           A=uold(active(ilevel)%igrid(i)+iskip,6)+uold(active(ilevel)%igrid(i)+iskip,nvar+1)
           B=uold(active(ilevel)%igrid(i)+iskip,7)+uold(active(ilevel)%igrid(i)+iskip,nvar+2)
           C=uold(active(ilevel)%igrid(i)+iskip,8)+uold(active(ilevel)%igrid(i)+iskip,nvar+3)
           e_mag=0.125d0*(A**2+B**2+C**2)
           e = e + e_mag

           ! Rebuild hydro variables from primitives
           uold(active(ilevel)%igrid(i)+iskip,1)=d
           uold(active(ilevel)%igrid(i)+iskip,2)=d*u
           uold(active(ilevel)%igrid(i)+iskip,3)=d*v
           uold(active(ilevel)%igrid(i)+iskip,4)=d*w
           uold(active(ilevel)%igrid(i)+iskip,5)=e
        end do
     end do
...
#ifdef NMAGTRACERS
     do kk=nvar+3+1,nvar+3+(6*NMAGTRACERS)
        call make_virtual_fine_dp(uold(1,kk),ilevel)
     enddo
#endif
#if NCR>0
     call make_virtual_fine_dp(uold(1,inener),ilevel)
#endif

  end do

end subroutine maghubble
!###########################################################                                                 
!###########################################################
!###########################################################
!###########################################################                                  
...
!################################################################
!################################################################
! This subroutine modifies the magnetic energy in a grid by
! injecting/extracting magnetic fields from the child cells.
! As the modification happens within the grid, no upl/upr call
! is required, and the modification is upload by simple
! conservative upload_fine
!
! Method: conservative toroids alignment
subroutine magMod_Toroids(igrid,emod,inj,clevel,nsn,emout,log_inject)
  ...
  logical,intent(in)      :: log_inject
  ! Cell ID variables #########################################
  integer                 :: clevel
  real(dp)                :: nsn  
  integer,intent(in)      :: igrid
  integer,dimension(1:8)  :: ind_cell_son
  integer                 :: ison,idim,iskip_son
  ! Injection variables #######################################
  integer                 :: inj, loopID
  real                    :: rn
  real(dp),intent(in)     :: emod
  real(dp)                :: binj, bmod
  real(dp)                :: emag0,emag1,eminj,emag0C,emag1C
  real(dp),intent(out)    :: emout
  logical                 :: noLower
  ! real(dp)                :: dx
  ! Useful parameters #########################################
  integer,parameter       :: neul = 5
  integer,parameter       :: nloops = 3
  real(dp),parameter      :: third = 0.3333333333333
#ifdef NMAGTRACERS
  integer                 :: nvarTr
  integer                 :: neulTr
#endif
...
#ifdef NMAGTRACERS
  nvarTr = nvar+6+6*(SN_mhdtrace-1)
  neulTr = nvar+3+6*(SN_mhdtrace-1)
#endif

  ! Compute scale quantities
  ! dx = 0.5d0**clevel
  ! call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  ! scale_B = sqrt(scale_d) * scale_l / scale_t
  emout = 0.0d0

  ! Generate cells ID's and identify refinements
  noLower=.true.
  do ison=1,twotondim
     iskip_son=ncoarse+(ison-1)*ngridmax
     ind_cell_son(ison)=iskip_son+igrid
     !write(*,*)"DB",igrid,clevel
     if (son(ind_cell_son(ison))>0) then
        noLower=.false.
     end if
  end do

  ! Inject when no refinements are found
  if (noLower.eq..true.) then
     ! Determine the magnetic field modification
     bmod = (inj*sqrt(2.0d0*emod/twotondim)/nloops)
     if(log_inject)then
      write(*,*)"Inject when no refinements are found",bmod
     endif
     ! Remove magnetic energy
     emag0C = 0.0d0
     do ison=1,twotondim
        emag0 = 0.125d0*( &
             & (uold(ind_cell_son(ison),neul+1) + uold(ind_cell_son(ison),nvar+1) )**2 + &
             & (uold(ind_cell_son(ison),neul+2) + uold(ind_cell_son(ison),nvar+2) )**2 + &
             & (uold(ind_cell_son(ison),neul+3) + uold(ind_cell_son(ison),nvar+3) )**2 )
        uold(ind_cell_son(ison),neul) = uold(ind_cell_son(ison),neul) - emag0
        emag0C = emag0C + emag0
     end do
     ! Randomized order injection
#ifdef NMAGTRACERS
     call random_number(rn)
     loopID = int(rn*nloops)+1
     call InjectLoopTr(loopID,bmod,ind_cell_son,neulTr,nvarTr)
     loopID = mod(loopID,nloops)+1
     call InjectLoopTr(loopID,bmod,ind_cell_son,neulTr,nvarTr)
     loopID = mod(loopID,nloops)+1
     call InjectLoopTr(loopID,bmod,ind_cell_son,neulTr,nvarTr)
#else
     call random_number(rn)
     loopID = int(rn*nloops)+1
     call InjectLoop(loopID,bmod,ind_cell_son)
     loopID = mod(loopID,nloops)+1
     call InjectLoop(loopID,bmod,ind_cell_son)
     loopID = mod(loopID,nloops)+1
     call InjectLoop(loopID,bmod,ind_cell_son)
#endif     
     ! Replenish magnetic energy
     emag1C = 0.0d0
     do ison=1,twotondim
        emag1 = 0.125d0*( &
             & (uold(ind_cell_son(ison),neul+1) + uold(ind_cell_son(ison),nvar+1) )**2 + &
             & (uold(ind_cell_son(ison),neul+2) + uold(ind_cell_son(ison),nvar+2) )**2 + &
             & (uold(ind_cell_son(ison),neul+3) + uold(ind_cell_son(ison),nvar+3) )**2 )
        uold(ind_cell_son(ison),neul) = uold(ind_cell_son(ison),neul) + emag1
        emag1C = emag1C + emag1
     end do
     eminj = emag1C-emag0C
     !write(*,*)"eminj",eminj,emag1C,emag0C
     ! If injection has failed, repeat to ensure injection!
     if (eminj .lt. 0.0d0) then
        ! bmod = -2.0d0*bmod 
        ! Remove magnetic energy
        do ison=1,twotondim
           emag0 = 0.125d0*( &
     ...
     emout = eminj
     !write(*,'("Injection not failed")')          
     !write(*,*)"eminj",eminj
     !write(*,*)"emout",emout
  else
     if (log_inject) write(*,'("Injection failed: found refined cell")')          
  end if
end subroutine magMod_Toroids
!################################################################
!################################################################
!################################################################
!################################################################
#ifdef NMAGTRACERS
subroutine InjectLoopTr(loopID,bmod,ind_cell_son,neulTr,nvarTr)
  use amr_commons
  use hydro_commons
  use hydro_parameters
  implicit none
  ...
  if (loopID.eq.1) then ! #### Compute XY loops #######################################
     ! ######################
     ! Compute XY-bottom loop
     ! 1,2,3,4; 1,2
     bloop = &
          & uold(ind_cell_son(1),nvar+1) + uold(ind_cell_son(2),nvar+2) - &
          & uold(ind_cell_son(4),neul+1) - uold(ind_cell_son(3),neul+2)
     if (bloop.gt.0) then
        binj = bmod
     else
        binj = -1.d0*bmod
     end if
     uold(ind_cell_son(1),nvar+1) = uold(ind_cell_son(1),nvar+1) + binj
     uold(ind_cell_son(2),neul+1) = uold(ind_cell_son(2),neul+1) + binj
     uold(ind_cell_son(2),nvar+2) = uold(ind_cell_son(2),nvar+2) + binj
     uold(ind_cell_son(4),neul+2) = uold(ind_cell_son(4),neul+2) + binj
     uold(ind_cell_son(4),neul+1) = uold(ind_cell_son(4),neul+1) - binj
     uold(ind_cell_son(3),nvar+1) = uold(ind_cell_son(3),nvar+1) - binj
     uold(ind_cell_son(3),neul+2) = uold(ind_cell_son(3),neul+2) - binj
     uold(ind_cell_son(1),nvar+2) = uold(ind_cell_son(1),nvar+2) - binj     
     uold(ind_cell_son(1),nvarTr+1) = uold(ind_cell_son(1),nvarTr+1) + binj
     uold(ind_cell_son(2),neulTr+1) = uold(ind_cell_son(2),neulTr+1) + binj
     uold(ind_cell_son(2),nvarTr+2) = uold(ind_cell_son(2),nvarTr+2) + binj
     uold(ind_cell_son(4),neulTr+2) = uold(ind_cell_son(4),neulTr+2) + binj
     uold(ind_cell_son(4),neulTr+1) = uold(ind_cell_son(4),neulTr+1) - binj
     uold(ind_cell_son(3),nvarTr+1) = uold(ind_cell_son(3),nvarTr+1) - binj
     uold(ind_cell_son(3),neulTr+2) = uold(ind_cell_son(3),neulTr+2) - binj
     uold(ind_cell_son(1),nvarTr+2) = uold(ind_cell_son(1),nvarTr+2) - binj             
     ! ######################
     ! Compute XY-top loop
     ! 5,6,7,8; 1,2
     bloop = &
          & uold(ind_cell_son(5),nvar+1) + uold(ind_cell_son(6),nvar+2) - &
          & uold(ind_cell_son(8),neul+1) - uold(ind_cell_son(7),neul+2)
     if (bloop.gt.0) then
        binj = bmod
     else
        binj = -1.d0*bmod
     end if
     uold(ind_cell_son(5),nvar+1) = uold(ind_cell_son(5),nvar+1) + binj
     uold(ind_cell_son(6),neul+1) = uold(ind_cell_son(6),neul+1) + binj
     uold(ind_cell_son(6),nvar+2) = uold(ind_cell_son(6),nvar+2) + binj
     uold(ind_cell_son(8),neul+2) = uold(ind_cell_son(8),neul+2) + binj
     uold(ind_cell_son(8),neul+1) = uold(ind_cell_son(8),neul+1) - binj
     uold(ind_cell_son(7),nvar+1) = uold(ind_cell_son(7),nvar+1) - binj
     uold(ind_cell_son(7),neul+2) = uold(ind_cell_son(7),neul+2) - binj
     uold(ind_cell_son(5),nvar+2) = uold(ind_cell_son(5),nvar+2) - binj
     uold(ind_cell_son(5),nvarTr+1) = uold(ind_cell_son(5),nvarTr+1) + binj
     uold(ind_cell_son(6),neulTr+1) = uold(ind_cell_son(6),neulTr+1) + binj
     uold(ind_cell_son(6),nvarTr+2) = uold(ind_cell_son(6),nvarTr+2) + binj
     uold(ind_cell_son(8),neulTr+2) = uold(ind_cell_son(8),neulTr+2) + binj
     uold(ind_cell_son(8),neulTr+1) = uold(ind_cell_son(8),neulTr+1) - binj
     uold(ind_cell_son(7),nvarTr+1) = uold(ind_cell_son(7),nvarTr+1) - binj
     uold(ind_cell_son(7),neulTr+2) = uold(ind_cell_son(7),neulTr+2) - binj
     uold(ind_cell_son(5),nvarTr+2) = uold(ind_cell_son(5),nvarTr+2) - binj
  else if (loopID.eq.2) then ! Compute XZ loops #######################################
     ! ######################
     ! Compute XZ-back loop
     bloop = &
          & uold(ind_cell_son(1),nvar+1) + uold(ind_cell_son(2),nvar+3) - &
          & uold(ind_cell_son(6),neul+1) - uold(ind_cell_son(5),neul+3)
     if (bloop.gt.0) then
        binj = bmod
     else
        binj = -1.d0*bmod
     ...
     uold(ind_cell_son(1),nvar+1) = uold(ind_cell_son(1),nvar+1) + binj
     uold(ind_cell_son(2),neul+1) = uold(ind_cell_son(2),neul+1) + binj
     uold(ind_cell_son(2),nvar+3) = uold(ind_cell_son(2),nvar+3) + binj
     uold(ind_cell_son(6),neul+3) = uold(ind_cell_son(6),neul+3) + binj
     uold(ind_cell_son(6),neul+1) = uold(ind_cell_son(6),neul+1) - binj
     uold(ind_cell_son(5),nvar+1) = uold(ind_cell_son(5),nvar+1) - binj
     uold(ind_cell_son(5),neul+3) = uold(ind_cell_son(5),neul+3) - binj
     uold(ind_cell_son(1),nvar+3) = uold(ind_cell_son(1),nvar+3) - binj     
     uold(ind_cell_son(1),nvarTr+1) = uold(ind_cell_son(1),nvarTr+1) + binj
     uold(ind_cell_son(2),neulTr+1) = uold(ind_cell_son(2),neulTr+1) + binj
     uold(ind_cell_son(2),nvarTr+3) = uold(ind_cell_son(2),nvarTr+3) + binj
     uold(ind_cell_son(6),neulTr+3) = uold(ind_cell_son(6),neulTr+3) + binj
     uold(ind_cell_son(6),neulTr+1) = uold(ind_cell_son(6),neulTr+1) - binj
     uold(ind_cell_son(5),nvarTr+1) = uold(ind_cell_son(5),nvarTr+1) - binj
     uold(ind_cell_son(5),neulTr+3) = uold(ind_cell_son(5),neulTr+3) - binj
     uold(ind_cell_son(1),nvarTr+3) = uold(ind_cell_son(1),nvarTr+3) - binj     
     ! ######################
     ! Compute XZ-front loop
     bloop = &
          & uold(ind_cell_son(3),nvar+1) + uold(ind_cell_son(4),nvar+3) - &
          & uold(ind_cell_son(8),neul+1) - uold(ind_cell_son(7),neul+3)
     if (bloop.gt.0) then
        binj = bmod
     else
        binj = -1.d0*bmod
     ...
     uold(ind_cell_son(3),nvar+1) = uold(ind_cell_son(3),nvar+1) + binj
     uold(ind_cell_son(4),neul+1) = uold(ind_cell_son(4),neul+1) + binj
     uold(ind_cell_son(4),nvar+3) = uold(ind_cell_son(4),nvar+3) + binj
     uold(ind_cell_son(8),neul+3) = uold(ind_cell_son(8),neul+3) + binj
     uold(ind_cell_son(8),neul+1) = uold(ind_cell_son(8),neul+1) - binj
     uold(ind_cell_son(7),nvar+1) = uold(ind_cell_son(7),nvar+1) - binj
     uold(ind_cell_son(7),neul+3) = uold(ind_cell_son(7),neul+3) - binj
     uold(ind_cell_son(3),nvar+3) = uold(ind_cell_son(3),nvar+3) - binj     
     uold(ind_cell_son(3),nvarTr+1) = uold(ind_cell_son(3),nvarTr+1) + binj
     uold(ind_cell_son(4),neulTr+1) = uold(ind_cell_son(4),neulTr+1) + binj
     uold(ind_cell_son(4),nvarTr+3) = uold(ind_cell_son(4),nvarTr+3) + binj
     uold(ind_cell_son(8),neulTr+3) = uold(ind_cell_son(8),neulTr+3) + binj
     uold(ind_cell_son(8),neulTr+1) = uold(ind_cell_son(8),neulTr+1) - binj
     uold(ind_cell_son(7),nvarTr+1) = uold(ind_cell_son(7),nvarTr+1) - binj
     uold(ind_cell_son(7),neulTr+3) = uold(ind_cell_son(7),neulTr+3) - binj
     uold(ind_cell_son(3),nvarTr+3) = uold(ind_cell_son(3),nvarTr+3) - binj     
  else if (loopID.eq.3) then ! Compute YZ loops #######################################
     ! ######################
     ! Compute YZ-left loop
     bloop = &
          & uold(ind_cell_son(1),nvar+2) + uold(ind_cell_son(3),nvar+3) - &
          & uold(ind_cell_son(7),neul+2) - uold(ind_cell_son(5),neul+3)
     if (bloop.gt.0) then
        binj = bmod
     else
        binj = -1.d0*bmod
     ...
     uold(ind_cell_son(1),nvarTr+3) = uold(ind_cell_son(1),nvarTr+3) - binj     
     ! ######################
     ! Compute YZ-right loop
     bloop = &
          & uold(ind_cell_son(2),nvar+2) + uold(ind_cell_son(4),nvar+3) - &
          & uold(ind_cell_son(8),neul+2) - uold(ind_cell_son(6),neul+3)
     if (bloop.gt.0) then
        binj = bmod
     else
        binj = -1.d0*bmod
     ...
     write(*,*) "Error injecting"   
  end if
end subroutine InjectLoopTr
#endif
!################################################################
!################################################################
!################################################################
!################################################################
subroutine InjectLoop(loopID,bmod,ind_cell_son)
  use amr_commons
  use hydro_commons
  use hydro_parameters
  implicit none
