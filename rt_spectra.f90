...
!-------------------------------------------------------------------------
  ...
  real*8:: integrateSpectrum, X(N), Y(N), e0, e1
  integer :: N, species
  interface
     real*8 function func(wavelength,intensity,species)
       real*8::wavelength,intensity
       integer::species
     end function func
  end interface!----------------------------------------------------------
  real*8,dimension(:),allocatable:: xx, yy, f
  real*8:: la0, la1
  integer :: i
...
!-------------------------------------------------------------------------
  ...
  real*8:: trapz1
  real*8:: X(N),Y(N)
  real*8,optional::cum(N)
  real*8,allocatable::cumInt(:)
!-------------------------------------------------------------------------
  allocate(cumInt(N))
  cumInt(:)=0.d0
  if (N.le.1) RETURN
...
!------------------------------------------------------------------------
  ...
  real*8      :: lambda, getCrosssection_Hui
  integer           :: species
  real*8      :: E, E0, cs0, P, ya, yw, y0, y1, x, y
!------------------------------------------------------------------------
...
!_________________________________________________________________________
  ...
  real*8,parameter::SED_dlgA=0.02d0
  real*8::SED_dlgZ
  real*8::SED_lgA0, SED_lgZ0
  real*8,allocatable,dimension(:)::SED_ages,SED_zeds![Gyr],[m_met/m_gas]
  ...
  real*8,allocatable,dimension(:,:,:,:)::SED_table
...
!-------------------------------------------------------------------------
  ...
  real*8,allocatable::ages(:), Zs(:), Ls(:), rebAges(:)
  real*8,allocatable::SEDs(:,:,:)           ! SEDs f(lambda,age,met)
  real*8,allocatable::tbl(:,:,:), tbl2(:,:,:), reb_tbl(:,:,:)
  integer::i,ia,iz,ip,ii,dum
  character(len=128)::fZs, fAges, fSEDs                        ! Filenames
  logical::ok,okAge,okZ
  real*8::dlgA, pL0, pL1, tmp
...
!-------------------------------------------------------------------------
  ...
  real*8,save,allocatable,dimension(:)::  L_star
  real*8,save,allocatable,dimension(:,:)::csn_star, cse_star
  real*8,save,allocatable,dimension(:)::  egy_star
  real*8,save,allocatable,dimension(:)::  sum_L_cpu,sum_L_all
  real*8,save,allocatable,dimension(:,:)::sum_csn_cpu,sum_csn_all
  real*8,save,allocatable,dimension(:,:)::sum_cse_cpu,sum_cse_all
  real*8,save,allocatable,dimension(:)::sum_egy_cpu,sum_egy_all
  real*8:: mass, age, Z, t_sne_Gyr
!-------------------------------------------------------------------------
  ...
  do ip=1,nSEDgroups
     ...
     if(sum_L_all(ip) .gt. 0.) then
        ...
        !write(*,*)"line628",group_egy(ip), group_csn(ip,:), group_cse(ip,:),sum_L_all(ip) 
...
!-------------------------------------------------------------------------
  ...
  use spectrum_integrator_module
  real*8:: getSEDLuminosity, X(n), Y(n), e0, e1
  integer :: N, species
  real*8,parameter :: const=1.0e-8/hp/c_cgs
  real*8,parameter :: Lsun=3.8256d33 ! Solar luminosity [erg/s]
...
!-------------------------------------------------------------------------
  use rt_parameters,only:c_cgs, eV_to_erg, hp
  use spectrum_integrator_module
  real*8:: getSEDEgy, X(N), Y(N), e0, e1, norm
  integer :: N,species
  real*8,parameter :: const=1.d8*hp*c_cgs/eV_to_erg! energy conversion
!-------------------------------------------------------------------------
  species      = 1                       ! irrelevant but must be included
...
!-------------------------------------------------------------------------
  ...
  real*8:: getSEDcsn, X(N), Y(N), e0, e1, norm
  integer :: N, species
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  use spectrum_integrator_module
  use rt_parameters,only:ionEVs
  real*8:: getSEDcse, X(N), Y(N), e0, e1, norm
  integer :: N, species
!-------------------------------------------------------------------------
  if(e1 .gt. 0. .and. e1 .le. ionEvs(species)) then
...
!-------------------------------------------------------------------------
  real*8:: xint_log, yint_log
  integer,intent(in):: nx, ny, nz
  integer::new_nx, new_ny
  real*8,intent(in):: x(nx),y(ny)
  real*8,intent(in):: data(nx,ny,nz)
  real*8,dimension(:,:,:),allocatable:: new_data
  real*8,dimension(:),allocatable:: new_x, new_lgx, new_y, new_lgy
  real*8:: dx0, dx1, dy0, dy1, x_step, y_step
  real*8:: x0lg, x1lg, y0lg, y1lg
...
!-------------------------------------------------------------------------
  use amr_commons
  use rt_parameters
  real*8, intent(in):: age, Z
  real*8:: lgAge, lgZ
  integer:: nProp
  logical:: same
  real*8,dimension(:):: ret
  integer,save:: ia, iz
  real*8,save:: da, da0, da1, dz, dz0, dz1
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  use rt_parameters
  real*8,intent(in):: age1_Gyr, dt_Gyr, Z
  real*8,dimension(nSEDgroups):: ret, Lc0, Lc1, SEDegy
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  ...
  real*8::dx,dx_loc,scale,vol_loc
  logical::error
  ! Grid based arrays
  real*8,dimension(1:nvector,1:ndim),save::x0
  ...
  real*8,dimension(1:nvector,ngroups),save::part_NpInp
  real*8,dimension(1:nvector,1:ndim),save::x
  integer ,dimension(1:nvector,3),save::id=0,igd=0,icd=0
  integer ,dimension(1:nvector),save::igrid,icell,indp,kg
  real*8,dimension(1:3)::skip_loc
  ! units and temporary quantities
  real*8::age, z, scale_inp, scale_Nphot, dt_Gyr, dt_loc_Gyr &
            , scale_msun, mass, t_sne_Gyr
  !real*8, dimension(nSEDgroups) :: ret
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v, scale_Np  & 
            , scale_Fp
  real*8,parameter::vol_factor=2**ndim   ! Vol factor for ilevel-1 cells
#ifdef NTRACEGROUPS
  integer::dumpid
  real*8::adiff,adiffnew,nptot
  real*8,dimension(1:ntraceGroups)::npfrac
...
SUBROUTINE get_tracer_group_from_halo(pos_star,groupindex,minind1)
  ...
  use rt_hydro_commons
  use rt_parameters
  real*8,dimension(1:ndim)::pos_star,xdist
  integer::groupindex,minind1, hcount, i, mindcount, mindrcount
  real*8::distance,donrvir,mindist,minrdist
...
!_________________________________________________________________________
  ...
  real*8,allocatable,dimension(:,:,:)::UV_groups_table
  ...
  real*8,parameter::fourPi=12.566371
  ! ----------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  ...
  real*8,allocatable  :: Ls(:)            ! Wavelengths
  real*8,allocatable  :: UV(:,:)          ! UV f(lambda,z)
  real*8,allocatable  :: tbl(:,:), tbl2(:,:)
  integer::i,ia,iz,ip,ii,dum,locid,ncpu2,ierr
  logical::ok
  real*8::da, dz, pL0,pL1
...
!-------------------------------------------------------------------------
  use amr_parameters
  real*8, intent(in):: z
  real*8:: ret(nIons,2), dz0, dz1, zr_factor
  integer:: iz0, iz1
...
!-------------------------------------------------------------------------
  use amr_parameters
  real*8, intent(in):: z
  real*8:: ret(nUVgroups,2+2*nIons), dz0, dz1, zr_factor
  integer:: iz0, iz1
  logical, optional, intent (in) :: z_damp
!-------------------------------------------------------------------------
  ret=0. ; if(z .gt. UV_maxz) RETURN
...
!-------------------------------------------------------------------------
  ...
  implicit none
  integer::i
  real*8,allocatable,save::UVprops(:,:) !Each group: flux, egy, csn, cse
...
!-------------------------------------------------------------------------
  real*8:: getUV_Hrate, X(N), Y(N), e0
  integer :: N, species
  real*8,parameter :: const1=fourPi*1.d8*hp*c_cgs
  real*8,parameter :: const2=fourPi*eV_to_erg
!-------------------------------------------------------------------------
  e0=ionEvs(species)
  getUV_Hrate = &
...
!-------------------------------------------------------------------------
  real*8:: getUVflux, X(N), Y(N), e0, e1
  integer :: N, species
!-------------------------------------------------------------------------
  species          = 1                   ! irrelevant but must be included
...
!-------------------------------------------------------------------------
  real*8:: getUVEgy, X(N), Y(N), e0, e1, norm
  integer :: N,species
  real*8,parameter :: const=1.d8*hp*c_cgs/eV_to_erg    ! unit conversion
!-------------------------------------------------------------------------
  species      = 1                       ! irrelevant but must be included
  norm         = integrateSpectrum(X, Y, N, e0, e1, species, f1)
  getUVEgy  = const * &
...
! Species is a code for the ion in question: 1=HI, 2=HeI, 3=HeIII
!-------------------------------------------------------------------------
  real*8:: getUVcsn, X(N), Y(N), e0, e1, norm
  integer :: N, species
!-------------------------------------------------------------------------
  if(e1 .gt. 0. .and. e1 .le. ionEvs(species)) then
...
! Species is a code for the ion in question: 1=HI, 2=HeI, 3=HeIII
!-------------------------------------------------------------------------
  real*8:: getUVcse, X(N), Y(N), e0, e1, norm
  integer :: N, species
!-------------------------------------------------------------------------
  if(e1 .gt. 0. .and. e1 .le. ionEvs(species)) then
...
!-------------------------------------------------------------------------
  real*8, intent(in)::xax(nx), x
  real*8:: x_step, dx0, dx1
  integer:: nx, ix0, ix1
!-------------------------------------------------------------------------
