module pm_parameters
  ...
  integer,parameter::nsinkmax_safe=2000 ! Maximum number of sinks (do not change!)
  ...
  logical::floor_acc=.true.                  ! Introduces a density floor in the accretion routines
  real(dp)::rho_min_acc=1d-31                ! Minimum accretion density (in g/cc)
  ...
  real(dp)::AGN_fbk_frac_RT=0               ! Fraction of AGN feedback released as radiation injection
  real(dp)::AGN_fbk_frac_mhd=0               ! Fraction of AGN feedback released as magnetic injection
  ...
  ! Detailed SMBH evolution report
  logical,parameter::storeSMBH=.true.
  logical          ::detailedSMBH=.true.

  ! Center of mass to seed SMBH 
  !integer::nsink_com=0                 ! Max. number of SMBH for com seeding
  !logical::seed_smbh_com=.false.       ! Activate com SMBH seeding
  !real(dp)::aexp_sink_com=1.0d0        ! Min. scale factor com SMBH seeding
