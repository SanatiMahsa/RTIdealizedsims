subroutine init_sink
  ...
  logical::sink_restart_output=.false.
  ...
  allocate(rinfsink(1:nsinkmax))
  allocate(wemag(1:nsinkmax))
  allocate(wemag_new(1:nsinkmax))
  allocate(wmetal(1:nsinkmax))
  allocate(wmetal_new(1:nsinkmax))
  allocate(lmom_gas(1:nsinkmax,1:ndim))
  allocate(lmom_gas_new(1:nsinkmax,1:ndim))
  allocate(lmom_gas_all(1:nsinkmax,1:ndim))
  allocate(eth_sink(1:nsinkmax))
  allocate(emag_sink(1:nsinkmax))
  allocate(agn_inj_mag(1:nsinkmax))
  allocate(agn_inj_tot(1:nsinkmax))
  allocate(metal_sink(1:nsinkmax))
  allocate(weighted_emag(1:nsinkmax,1:nlevelmax))
  allocate(weighted_metal(1:nsinkmax,1:nlevelmax))  

  ! Hugo's dynamical friction from particles
  if (drag_part) then
  ...
  if(nrestart>0)then
     ...
     INQUIRE(FILE=fileloc, EXIST=sink_restart_output)
     if(sink_restart_output)then
...
subroutine compute_ncloud_sink
  ...
  ! ncloud_sink_massive=max(ncloud_sink_massive,1)
  ! ncloud_sink=max(ncloud_sink,1)
