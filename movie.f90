...
subroutine output_frame()
  ...
  character(len=100),dimension(0:nmovie_types) :: moviefiles
  integer::icell,ncache,iskip,irad,ngrid,nlevelmax_frame
  integer::nframes,rt_nframes,imap
  ...
  real(dp),parameter::tyoung_stars=100.0d0 ! 100 Myr
  real(dp),parameter::told_stars  =500.0d0 ! 500 Myr
  real(dp)::current_time,tyoung_movie,told_movie

  ! Inflow / outflow variables
  real(dp)::filter_number,flow_threshold,norm_dist
  real(dp),dimension(1:ndim)::xx_org,xx_com,vgas
 
  ! Dummy numbers for ID required maps
  integer::ipart_start
  integer::idm_map,istar_map,ilum_map,iyoungst_map,ioldst_map
  integer::iinflow_map,ioutflow_map,inoflow_map
  
#ifdef TRACKER
  integer::itrack
#endif  
  ! real(dp)::age,age_db1,age_db2,age_db3,age_db4

  nh_temp    = nh_frame
  nw_temp    = nw_frame
  !--------------------------------------------------   
  ! Determine movie dummy numbers
  !--------------------------------------------------   
  ! Grid maps
#ifdef SOLVERmhd
  iinflow_map=NVAR+5
...
#endif
  ! Particle maps
  ipart_start=nmovie_types-4
  idm_map=ipart_start
  istar_map=ipart_start+1
  ilum_map=ipart_start+2
  iyoungst_map=ipart_start+3
  ioldst_map=ipart_start+4
 ...
 do proj_ind=1,LEN(trim(proj_axis)) 
...
#ifdef TRACKER
  infofile=trim(moviedir)//'tracker_com_'//trim(istep_str)//'.txt'
  if(myid==1)call output_tracks(infofile)
#endif
#ifndef WITHOUTMPI
...
#if NCR>0
  moviefiles(9) = trim(moviedir)//'pcrs_'//trim(istep_str)//'.map'
#endif
#endif

  ! Maps with already determined identity
  moviefiles(iinflow_map) = trim(moviedir)//'inflw_'//trim(istep_str)//'.map'
  moviefiles(ioutflow_map) = trim(moviedir)//'outflw_'//trim(istep_str)//'.map'
  moviefiles(inoflow_map) = trim(moviedir)//'noflw_'//trim(istep_str)//'.map'

  moviefiles(idm_map) = trim(moviedir)//'dm_'//trim(istep_str)//'.map'
  moviefiles(istar_map) = trim(moviedir)//'stars_'//trim(istep_str)//'.map'
  moviefiles(ilum_map) = trim(moviedir)//'lum_'//trim(istep_str)//'.map'  
  moviefiles(iyoungst_map) = trim(moviedir)//'100Myr_'//trim(istep_str)//'.map'
  moviefiles(ioldst_map) = trim(moviedir)//'oldstr_'//trim(istep_str)//'.map'
  ...
  flow_threshold=vwind_movie*1.0d5/scale_v ! From km/s to code velocity
  
  ! Convert required ages to code
  if (use_proper_time)then
     tyoung_movie = tyoung_stars*myr2s/(scale_t/aexp**2)
     told_movie = told_stars*myr2s/(scale_t/aexp**2)
     current_time=texp
  else
     tyoung_movie = tyoung_stars*myr2s/scale_t
     told_movie = told_stars*myr2s/scale_t
     current_time=t
  end if
  tyoung_movie = current_time - tyoung_movie
  told_movie = current_time - told_movie
  ...
  if (track_center_mass) then
     if (do_center_track_movie(proj_ind).and.(aexp.gt.aexp_track_movie(proj_ind))) then
        itrack=itracker_movie(proj_ind)
        if (tracker(itrack)%active) then
           xcentre_frame(proj_ind*4-3)=tracker(itrack)%xpos(1)
           xcentre_frame(proj_ind*4-2)=0.0d0
           xcentre_frame(proj_ind*4-1)=0.0d0
           xcentre_frame(proj_ind*4-0)=0.0d0
           ycentre_frame(proj_ind*4-3)=tracker(itrack)%xpos(2)
           ycentre_frame(proj_ind*4-2)=0.0d0
           ycentre_frame(proj_ind*4-1)=0.0d0
           ycentre_frame(proj_ind*4-0)=0.0d0
           zcentre_frame(proj_ind*4-3)=tracker(itrack)%xpos(3)
           zcentre_frame(proj_ind*4-2)=0.0d0
           zcentre_frame(proj_ind*4-1)=0.0d0
           zcentre_frame(proj_ind*4-0)=0.0d0
        end if
     end if
  end if
#endif
     ...
     do ilevel=levelmin,nlevelmax_frame
        ...
        do igrid=1,ncache,nvector
           ...
           do ind=1,twotondim
              ...
              cell_loop: do i=1,ngrid
                 if(ok(i))then
#if NDIM>2                 
                    ! Compute position in tracker frame
                    xx_org=xx(i,:)
                    if (track_center_mass) then
                       itrack=itracker_movie(proj_ind)
                       if (tracker(itrack)%active) then
                          xx_com=xx_org-tracker(itrack)%xpos
                       else                                               
                          xx_com=xx_org-(/xcen,ycen,zcen/)
                       end if
                    else
                       xx_com=xx_org-(/xcen,ycen,zcen/)
                    end if
...
666                          continue
			     ...
			     imap = 1
                             do kk=0,ipart_start-1
                                if(movie_vars(kk).eq.1)then
                                   ...
                                   ! Density map case
                                   if( (kk.eq.1).or. &
                                        & (kk.eq.iinflow_map).or.(kk.eq.ioutflow_map).or.(kk.eq.inoflow_map) &
                                        & ) then
                                      uvar = uold(ind_cell(i),1)
                                      if ((kk.eq.iinflow_map).or.(kk.eq.ioutflow_map)) then
                                         vgas(:)=uold(ind_cell(i),2:ndim+1)
                                         ! Compute direction vector and movie frame velocity
                                         if (track_center_mass) then
                                            itrack=itracker_movie(proj_ind)
                                            if (tracker(itrack)%active) vgas=vgas-tracker(itrack)%vel
                                         end if
                                         
                                         ! Project vgas along position vector
                                         norm_dist=sqrt(sum(xx_com**2))
                                         filter_number=sum(vgas*xx_com)/norm_dist
                                         ! Determine inflow
                                         if (kk.eq.iinflow_map) then
                                            if (filter_number.ge.-1.0d0*flow_threshold) uvar = 0.0d0
                                         end if
                                         ! Determine outflow
                                         if (kk.eq.ioutflow_map) then
                                            if (filter_number.le.flow_threshold) uvar = 0.0d0
                                         end if
                                         ! Determine noflow
                                         if (kk.eq.inoflow_map) then
                                            if ((filter_number.lt.-1.0d0*flow_threshold) &
                                                 & .or. (filter_number.gt.flow_threshold) ) uvar = 0.0d0
                                         end if
                                      end if
                                   ! Other scalars map
...
#endif
#if NCR>0
                                   ! Cosmic rays energy map case
                                   if(kk.eq.9)then
                                      uvar = uold(ind_cell(i),9)
                                   endif
#endif                                   
                                   ! Frame update
                                   if(method_frame(proj_ind).eq.'min')then
  ...
  do j=1,npartmax
     ...
     ! Fill up map with projected mass
     imap = 1
     do kk=0,ipart_start+4
        if(movie_vars(kk).eq.1)then
           if(star) then
              ! DM particles
              if((tp(j).eq.0d0).and.(kk.eq.idm_map)) then
              ...
              ! Star particles
              if((tp(j).ne.0d0).and.(kk.eq.istar_map)) then
                 data_frame(ii,jj,imap)=data_frame(ii,jj,imap)+mp(j)
              endif
	      ...
	      ! The polynome is fitted on Starburst99 instantaneous bolomtetric magnitude
              ...
              Lum: if((tp(j).ne.0d0).and.(kk.eq.ilum_map)) then
		 ! Polynome is poorly constrained on high and low ends
		    ...
		    do npoly=1,size(lum_poly)
              ...
              ! Young star particles (100 Myr)
              if((tp(j).ne.0d0).and.(kk.eq.iyoungst_map)) then
                 ! call getStarAgeGyr(tp(j), age)
                 ! YoungPartsOnly: if (age.le.0.1d0) then
                 YoungPartsOnly: if (tp(j).ge.tyoung_movie) then
                    data_frame(ii,jj,imap)=data_frame(ii,jj,imap)+mp(j)
                 end if YoungPartsOnly
              end if

              ! Old star particles (500 Myr)
              if((tp(j).ne.0d0).and.(kk.eq.ioldst_map)) then
                 ! call getStarAgeGyr(tp(j), age)
                 ! OldPartsOnly: if (age.gt.0.5d0) then
                 OldPartsOnly: if (tp(j).le.told_movie) then
                    data_frame(ii,jj,imap)=data_frame(ii,jj,imap)+mp(j)
                 end if OldPartsOnly
              end if
  ...
  if(myid==1)then
     if(method_frame(proj_ind)(1:4).ne.'sum')then
        ...
        do ii=1,nw_frame
          do jj=1,nh_frame
            imap = 1
            ! Note: it would be preferable to volume-weight magnetic quantities (6,7,8,nvar+1,nvar+2,nvar+3,nvar+4)
            do kk=0,ipart_start
              if(movie_vars(kk).eq.1)then
...
#if NCR>0
  if(ANY(movie_vars_txt=='pcrs  ')) movie_vars(9)=1  
#endif
#endif

  ! Maps with dummy number already defined
  ! Grid maps
  if(ANY(movie_vars_txt=='inflw ')) movie_vars(iinflow_map)=1
  if(ANY(movie_vars_txt=='outflw')) movie_vars(ioutflow_map)=1
  if(ANY(movie_vars_txt=='noflw')) movie_vars(inoflow_map)=1
  ! Particle maps
  if(ANY(movie_vars_txt=='dm    ')) movie_vars(idm_map)=1
  if(ANY(movie_vars_txt=='stars ')) movie_vars(istar_map)=1
  if(ANY(movie_vars_txt=='lum   ')) movie_vars(ilum_map)=1
  if(ANY(movie_vars_txt=='100Myr')) movie_vars(iyoungst_map)=1
  if(ANY(movie_vars_txt=='oldstr')) movie_vars(ioldst_map)=1
