...
subroutine mechanical_feedback_fine(ilevel,icount)
  ...
  if (verbose) write(*,*)' Entering mechanical fine', ilevel
        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              if(pop3 .and. (zp(ipart).lt.Zcrit_pop3) ) then
                 ...
                 ! ok = ((idp(ipart).lt.0).and.(tp(ipart).ne.0))
                 ok = is_active_star(idp(ipart),mp(ipart),tp(ipart))
...
subroutine mech_fine(ind_grid,ind_pos_cell,np,ilevel,dteff,nSN,mSN,pSN,mZSN,nphSN,mchSN)
  ...
  real(dp),dimension(1:ndim)::xc_send
  ...
  real(dp)::d1,d2,d3,d4,d5,d6,dtot
  real(dp)::vturb,vth,Mach,sig_s2,dratio,mload_cen
#ifdef SOLVERmhd
  real(dp) :: pvar(1:nvar+3)
  real(dp) :: emag,ecrs
#else
  real(dp) :: pvar(1:nvar)
#endif 
...
#ifdef SOLVERmhd
  ! Variables for magnetic energy injection
  ! real(dp),allocatable :: partThT(:)
  real(dp),dimension(1:nvector)::partNsn
  real(dp)::fCR=0.0d0,fCRnei=0.0d0
  real(dp)::scale_E
  real(dp)::emaginject
  logical ::log_inject=.false.
  integer ::inject_mode
#else
  logical,parameter::log_inject=.false.
#endif
  real(dp)::E_SNIIeff ! Effective energy injected (in case some energy is used as cosmic rays instead)
#if NCR>0
  ! Cosmic ray injection variables
  real(dp)::ecrinject
  logical,parameter::log_cr=.false.
#endif
  real(dp),dimension(1:nvector)::dLog,TLog,zLog,crLog,magLog,criLog,magiLog,magriLog

  if(verbose) write(*,*) "Entering mech_fine"

  ! Conversion factor from user units to cgs units
  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  scale_msun = scale_l**3*scale_d/msun2g
  scale_E = (scale_l * scale_d * scale_l)
  scale_E = (scale_E * (scale_l/scale_t)**2) * scale_l
       
#if NCR>0
  ! Definitions required for the injection of CRs
  fCRnei = (injCRextNei + injCRgenNei)/(1.0d0 - injCRextNei)
  fCR    = injCRext    + injCRgen
  E_SNIIeff=E_SNII*(1.0d0 - injCRext - injCRextNei)
#else
  E_SNIIeff=E_SNII
#endif

#ifdef SOLVERmhd
  ! ! Magnetic energy injection quantities
  ! if (injBfac.gt.0.0d0) then
  !    allocate(partThT(np))
  !    partThT(:) = 0.0d0
  ! end if
  ! Prepare SN counting vector
  if ((injBfac.gt.0.0d0).or.(fCR.gt.0)) then
     partNsn(:) = 0
     dLog=0
     TLog=0
     zLog=0
     ...
     crLog=0
     criLog=0
     if (.not.log_mfb) log_inject=.false.
  else
     log_inject=.false.
  end if
#endif  
  ...
  do i=1,np
     ...
     xc_send=xc2(1:3,i)
     call get_icell_from_pos (xc_send, ilevel+1, igrid, icell,ilevel2)
...
#ifdef SOLVERmhd
     ! Save SN number
     if ((injBfac.gt.0.0d0).or.(fCR.gt.0)) partNsn(i) = num_sn
#endif
     ...
     emag = 0.0d0
     do idim=1,ndim  
        emag = emag + 0.125d0*(uold(icell,neul+idim) + uold(icell,nvar+idim))**2
     end do
     e = e - emag
#endif
...
!        stop
     ...
     if((Tk.lt.T2min).or.(Tk.lt.0.0d0))then
        Tk=T2min
        e=T2min*d/scale_T2/(gamma-1.0)*1.2195
        uold(icell,neul) = 0.5d0*d*(u**2+v**2+w**2) + e
#if NENER>0
        do irad=0,nener-1
           uold(icell,neul) = uold(icell,neul) + uold(icell,inener+irad)
        end do
#endif
#ifdef SOLVERmhd
        ! do idim=1,ndim
        !    uold(icell,neul) = uold(icell,neul) + 0.125d0*(uold(icell,neul+idim) + uold(icell,nvar+idim))**2
        ! end do
        uold(icell,neul) = uold(icell,neul) + emag
#endif
     endif
     ...
     if(log_mfb)then
        if (log_inject) then        
           dLog(i)=log10(d*scale_nH)
           TLog(i)=log10(Tk)
           zLog(i)=log10(z/0.02)
#ifdef SOLVERmhd
           if (fCR.gt.0.0d0) crLog(i) = log10(uold(icell,inener)*vol_loc*scale_E)
           if (injBfac.gt.0.0d0)  magLog(i) = log10(emag*vol_loc*scale_E)
#endif
        else
398        format('MFB z= ',f9.5,' N= ',f7.1,' nH,T,Z= ',3(f7.3,1x),' dx= ',f9.5)
     ...
     do j=1,nSNnei
        xc_send=xc2(1:3,i)+xSNnei(1:3,j)*dx
        call get_icell_from_pos (xc_send, ilevel+1, igrid, icell,ilevel2)
        if(cpu_map(father(igrid)).eq.myid) then ! if belong to myid
           ...
           vload_rad = dsqrt(2d0*f_ESN*E_SNIIeff*(1d0+f_w_crit)/(M_SN_var*msun2g))/scale_v/(1d0+f_w_cell)/f_LOAD/f_PCAN
           if(f_w_cell.ge.f_w_crit)then ! radiative phase
              ...
              vload = dsqrt(2d0*f_esn2*E_SNIIeff/(1d0+f_w_cell)/(M_SN_var*msun2g))/scale_v/f_LOAD
              if(mechanical_geen) then
...
#ifdef SOLVERmhd
     do idim=1,ndim
        e = e - 0.125d0*(uold(icell,neul+idim) + uold(icell,nvar+idim))**2 
     end do
#endif
...
#if NCR>0
     ! Inject CR energy to central cell
     if (fCR.gt.0.0d0) then
        ! ecrinject = num SN * fraction for CR * Energy/sn
        ecrinject = partNsn(i)*fCR*E_SNII/(scale_E*vol_loc)
        if (log_inject) criLog(i) = log10(partNsn(i)*fCR*E_SNII)
        uold(icell,inener) = uold(icell,inener) + ecrinject
        uold(icell,neul) = uold(icell,neul) + ecrinject
     end if
#endif
  ...
  do i=1,np
     ...
     do j=1,nSNnei
        ...
        if(cpu_map(father(igrid)).ne.myid) then ! need mpi
...
#ifdef SOLVERmhd
           if(ilevel>ilevel2)then ! touching level-1 cells
              pvar(nvar+1:nvar+3) = unew(icell,nvar+1:nvar+3)
           else
              pvar(nvar+1:nvar+3) = uold(icell,nvar+1:nvar+3)
           endif            
#endif
           ...
           emag=0.0d0
           do idim=1,ndim 
              emag = emag+0.125d0*(pvar(neul+idim) + pvar(nvar+idim))**2
           end do
           eth0 = eth0 - emag
#endif
           ! For stability
...
! #ifdef SOLVERmhd
!            if (injBfac.gt.0.0d0) partThT(i) = eth0
! #endif
           ...
           w   = pvar(4)/d
           ekk = 0.5*d*(u**2 + v**2 + w**2)
           pvar(neul) = max(etot0, ekk+eth0)
           
           
           ! sanity check
           Tk = (pvar(neul)-ekk)/d*scale_T2*(gamma-1)
           if(Tk<0)then
           ...
           ! due to mechanical feedback
           T2min=T2_star*(d*scale_nH/n_star)**(g_star-1.0)
           if ((Tk.lt.T2min).and.(Tk.lt.0.0d0)) then
              Tk=T2min
              e=T2min*d/scale_T2/(gamma-1.0)*1.2195
              pvar(neul) = e + ekk
           endif
...
#if NCR>0
           if (fCRnei.gt.0.0d0) pvar(inener) = pvar(inener)+fCRnei*ek_solid(i,j)/vol_nei ! Injection of energy into CRs           
#endif
#if NENER>0
          do irad=0,nener-1
             pvar(neul) = pvar(neul) + pvar(inener+irad)
           end do
#endif
#ifdef SOLVERmhd 
           do idim=1,ndim
              pvar(neul) = pvar(neul) + 0.125d0*(pvar(neul+idim) + pvar(nvar+idim))**2 
           end do
#endif
           if(metal)then
...
#ifdef SOLVERmhd
           if(ilevel>ilevel2)then ! touching level-1 cells
              unew(icell,nvar+1:nvar+3) = pvar(nvar+1:nvar+3)
           else
              uold(icell,nvar+1:nvar+3) = pvar(nvar+1:nvar+3)
           endif 
#endif
...
#ifdef SOLVERmhd
  !-------------------------------------------------------------
  ! Inject magnetic fields with SN explosion
  !-------------------------------------------------------------
  if (injBfac.gt.0.0d0) then
     do i=1,np
        ! Compute required magnetic energy
        emaginject=partNsn(i)*injBfac*E_SNII/(scale_E*vol_loc)
        if (log_inject) magiLog(i) = log10(partNsn(i)*injBfac*E_SNII)

        inject_mode=1
        ind_cell = ncoarse+ind_grid(i)+(ind_pos_cell(i)-1)*ngridmax
        do j=1,nSNnei
           call get_icell_from_pos (xc2(1:3,i)+xSNnei(1:3,j)*dx, ilevel+1, igrid, icell,ilevel2)
           CheckLocal: if(cpu_map(father(igrid)).ne.myid) then ! need mpi
              inject_mode=2
           else
              CheckUpper: if(ilevel>ilevel2)then ! touching level-1 cells
                 inject_mode=3
              end if CheckUpper
           end if CheckLocal
        end do
        InjectConditional: if(inject_mode.eq.1) then
           call magMod_Toroids(igrid,emaginject,1,ilevel,partNsn(i),emag,log_inject)
           if (log_inject) magriLog(i) = log10(emag*vol_loc*scale_E)
        else if (inject_mode.eq.2) then
           call magMod_Toroids(igrid,emaginject,1,ilevel,partNsn(i),emag,log_inject)
           if (log_inject) magriLog(i) = log10(emag*vol_loc*scale_E)
        else if (inject_mode.eq.3) then
           if (log_inject) then 
              write(*,*) "Injection failed due to de-refined nbor"
              magriLog(i) = -10.0d0
           end if
        end if InjectConditional
     end do
  end if
#endif

#ifdef SOLVERmhd
  ! Detailed MFB log for injection (magnetic fields and cosmic rays)
  if(log_inject.and.log_mfb) then
     do i=1,np
#if NCR>0
     ! with magnetic fields and cosmic rays
618  format('MFB z= ',f9.5,' N= ',f7.1,' nH,T,Z,eM,eCR= ',5(f7.3,1x),'eMi,eMir=',2(f7.3,1x),'eCRi=',1(f7.3,1x),'dx= ',f9.5)
     write(*,618) 1./aexp-1,partNsn(i),dLog(i),TLog(i),zLog(i),magLog(i),crLog(i),magiLog(i),magriLog(i),criLog(i),log10(dx_loc*scale_l/3.08d18)
#else
     ! with magnetic fields only
617  format('MFB z= ',f9.5,' N= ',f7.1,' nH,T,Z,eM= ',4(f7.3,1x),'eMi,eMir=',2(f7.3,1x),'dx= ',f9.5)
     write(*,617) 1./aexp-1,partNsn(i),dLog(i),TLog(i),zLog(i),magLog(i),magiLog(i),magriLog(i),log10(dx_loc*scale_l/3.08d18)
#endif
     end do
  end if
#endif
...
subroutine mech_fine_mpi(ilevel)
  ...
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  real(dp)::scale_msun,msun2g=2d33,etot0
  real(dp)::skip_loc(1:3),d,u,v,w,ekk,eth,d0,u0,v0,w0,eth0,ekk0,Tk0,ekk_ej,T2min
  integer::igrid,icell,ilevel2,irad,ii,ich
  real(dp),dimension(1:twotondim,1:ndim),save::xc
  real(dp),dimension(1:ndim)::xc_send
...
!  logical,dimension(1:nvector,1:nSNnei),save ::snowplough
  ...
  do i=1,np
     ...
     do j=1,nSNnei
        ...
        if(cpu_map(father(igrid)).eq.myid) then ! if belong to myid
           ...
           vload_rad = dsqrt(2d0*f_ESN*E_SNIIeff*(1d0+f_w_crit)/(M_SN_var*msun2g))/(1d0+f_w_cell)/scale_v/f_LOAD/f_PCAN
           if(f_w_cell.ge.f_w_crit)then ! radiative phase
              vload = vload_rad
              snowplough(i,j)=.true.
           else ! adiabatic phase
  ...
  do i=1,np
     ...
     do j=1,nSNnei
        xc_send=xSN_i(1:3)+xSNnei(1:3,j)*dx
        call get_icell_from_pos (xc_send, ilevel+1, igrid, icell, ilevel2)
...
#ifdef SOLVERmhd
           do idim=1,ndim
              eth0 = eth0 - 0.125d0*(pvar(neul+idim) + pvar(nvar+idim))**2
           end do
#endif
           ! For stability
